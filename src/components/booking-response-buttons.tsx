...
export const BookingResponseButtons: React.FC<Props> = ({ bookingId }) => {
  ...
  const { status: updateStatus, request: setBookingStatus } = useSetBookingStatus();
  ...
  const handleRejectPress = () => {
    Alert.alert("Reject booking?", "Are you sure you want to reject the booking?", [
      {
        text: "Cancel",
        style: "cancel",
      },
  ...
  const handleConfirmPress = () => {
    Alert.alert("Confirm booking", "Confirm this booking? This will charge the customer", [
      {
        text: "Cancel",
        style: "cancel",
      },
  ...
  return (
    <View style={styles.container}>
      ...
      {updateStatus === 'error' && <Text>There was an error. Please try again later</Text>}
    </View>
  )
}
